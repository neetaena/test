<?php
		...
		if(empty($_GET['excel'])){
			print("<div class=\"container\">");
				print("<a href=\"excel_report_gas_ngv.php?datein=".$datein."&dateout=".$dateout."&excel=1&today=2&type_fule=$type_fule\" target=\"blank\" style=\"cursor:pointer;color:#43ad04;float:right;margin-top:10px;\" class=\"excel\"> <img src=\"image/excel.png\" title=\"Export to excel\" width=\"50px\" height=\"50\"></a> ");
			print("</div>");
		}

				$boonpon_id_data = array();
				$get_boonpon = getlist("SELECT p.boonpon_id FROM production_order as p inner join insertdata_transport as it on p.boonpon_id=it.boonpon_id inner join car_detail as c on it.idcar=c.id_car WHERE delivery_date between '".$datein."' and '$dateout' and typefule='$type_fule'  and type_site='1' GROUP BY boonpon_id");
			...
			print("<table  bgcolor =\"#FFFFFF\" border = \"1\" cellspacing = \"0\" cellpadding = \"2\" align = \"center\" valign = \"middle\" style=\"width:350mm;empty-cells: show;\">");
				print("<tr>");
					...
					print("<td align = \"center\" class='font-style' style='width:5%;' rowspan='2'><b>");
						print("แก๊สที่เติม(Kg)");
					print("</td>");
					print("<td align = \"center\" class='font-style' style='width:5%;' rowspan='2'><b>");
						print("ระยะทางวิ่ง(Km)");
					print("</td>");
		...
		//---------------------------------------------------------------ทะเบียน---------------------------------------------------------------
							...
							if(empty($get_data)){
								...
								print("<a onclick = \"window.open('add/add_note_driver.php?license=".$get_car_detail[0]['licenceplates']."&note_date=".$get_data[0]['delivery_date']."' , '','nenuber=no,toorlbar=no,location=no,scrollbars=no, status=no,resizable=no,width=600,height=300,top=45 ,left=250') \" style=\"cursor:pointer;color:#000;\">".$note_data[0]." ".$note_data[1]."</a> ");
		...
		//---------------------------------------------------------------สถานที่จัดส่ง---------------------------------------------------------------
			...
			//------------------------------------------นน.บรรทุก-------------------------
					...
					//---------------------------------------------------------------ประเภทรถ---------------------------------------------------------------
							...
							print("<td class='font-style' style='text-align:center;'>");
							$total_motor_way += $check_data[0]['motor_way'];
									print($check_data[0]['motor_way']);

							print("</td>");

							print("<td class='font-style' style='text-align:center;'>");
							$total_repair += $check_data[0]['repair'];
									print($check_data[0]['repair']);
		...
		//--------------------------------------------------ปั้มแก๊ซ--------------------------------------
				...
				print("<tr>");
					print("<td class='font-style' style='text-align:center;' colspan='6'>");
						...
						//--------------บัตรเครดิต---
						print("<td class='font-style' style='text-align:center;'>");
							print(number_format($total_motor_way,2));
						print("</td>");
						//--------------ใช้เงินสด.---
						print("<td class='font-style' style='text-align:center;'>");
							print(number_format($total_repair,2));
						print("</td>");
						//---------------มาตราฐาน.---
						/*print("<td class='font-style' style='text-align:center;'>");
