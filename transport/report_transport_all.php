...
global $getdata;
	...
	<table  bgcolor = "#FFFFFF" border = "0" cellspacing = "0" cellpadding = "2" align = "center" valign = "middle" style="width:180mm;empty-cells: show;padding-top: 18px;">
		<tr>
	...
	<table  bgcolor = "#FFFFFF" border = "0" cellspacing = "0" cellpadding = "2" align = "center" valign = "middle" style="width:180mm;empty-cells: show;">
		...
		<tr>
			...
			<td align = "left" rowspan="3" style = "width:40mm;" class="header_report">
				<b>*** พนักงานขับรถต้องตรวจสอบเอกสารที่ต้องนำกลับโรงงานและต้องมีลายมือชื่อลูกค้าที่รับสินค้าให้ครบถ้วนทุกครั้ง***</b>
			</td>
		</tr>
	...
	$get_boonpon_id = getlist("SELECT * FROM production_order  WHERE delivery_date = '".$_GET['delivery_date']."' and product_id='".$_GET['product']."' and boonpon_id !='' group by boonpon_id");

	for ($g=0; $g < sizeof($get_boonpon_id); $g++) { 
	
	$getdata = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production  WHERE boonpon_id = '".$get_boonpon_id[$g]['boonpon_id']."' and delivery_date = '".$_GET['delivery_date']."' ");
	...
	//ใช้เมื่อจำนวน Item มากกว่า 16 เพราะล้นช่องที่กำหนดไว้
