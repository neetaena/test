...
<html>
	...
	<?php
		...
		function headdetail(){
			print("<table  bgcolor =\"#FFFFFF\" border = \"1\" cellspacing = \"0\" cellpadding = \"2\" align =\"center\" valign = \"middle\" style=\"width:250mm;empty-cells: show;border: 1px groove;\">");
			print("<tr >");
	...
	<?php
			...
			$count = 1;
			$getdata = getlist("SELECT * FROM production_order WHERE $where_data  GROUP BY number order by delivery_date asc");
			if(!empty($getdata))
			{
			...
			for($i=0;$i<sizeof($getdata);$i++){
				$m = 1;
				$get_new = getlist("SELECT * FROM production_order WHERE number ='".$getdata[$i]['number']."'");

				for ($g=0; $g < sizeof($get_new); $g++) { 
					if($m==1)
					{
				print("<tr>");
				print("<td style=\"width:15mm;text-align:center;\" class=\"body_table\"  rowspan=\"".sizeof($get_new)."\">");
					//print $get_new[$g]['item_number']; //เลขที่ใบสั่งขาย
					print  printShortNumDate($get_new[$g]['crate_date']); //เลขที่ใบสั่งขาย
					//print printShortNumDate($get_new[$g]['delivery_date']); //ทดสอบ
				print("</td>");
				print("<td style=\"width:15mm;text-align:center;\" class=\"body_table\"  rowspan=\"".sizeof($get_new)."\">");
				...
				print("</td>");
				print("<td style=\"width:25mm;\" class=\"body_table\"  rowspan=\"".sizeof($get_new)."\">");
				...
				print("<td style=\"width:10mm;text-align:center;\" class=\"body_table\">");
					print("");
				print("</td>");
				print("<td style=\"width:35mm;\" class=\"body_table\">");
