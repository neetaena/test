...
<html>
	...
	<?php
		...
		print("<table  bgcolor = \"#FFFFFF\" border = \"0\" cellspacing = \"0\" cellpadding = \"2\" align = \"center\" valign = \"middle\" style=\"width:180mm;empty-cells: show;\">");
			print("<tr>");
				...
				print("บริษัท วนชัย กรุ๊ป จำกัด (มหาชน)<br>รายงานสินค้ารอจัดส่ง");
					print $wood;
				print("</td>");
			print("</tr>");
			print("<tr>");
	...
	$search[2] = !empty($product_id) ? "boonpon_id  is null" : "";
	...
	$get_type = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production WHERE $where_data  group by product_id ");
		for ($ty=0; $ty <sizeof($get_type) ; $ty++) { 
			...
			$getdata1 = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production WHERE delivery_date ='".$cut_new_date[0]."'  and  product_id='".$get_type[$ty]['product_id']."' and boonpon_id  is null group by warehouse_id    order by invoice asc");
			
			$next =1;
			for($i=0;$i<sizeof($getdata1);$i++)
			{
				$m = 1;
				$get_new = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production  WHERE warehouse_id ='".$getdata1[$i]['warehouse_id']."' and  product_id='".$get_type[$ty]['product_id']."' and delivery_date ='".$cut_new_date[0]."'  and boonpon_id  is null");
				$total_quantity = 0;
				$total_court=0;
				$total_q = 0;
