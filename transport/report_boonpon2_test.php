...
/*print("<div class=\"container\">");
				print("<a href=\"excel_report_boonpon2.php?datein=".$datein."&dateout=".$dateout."&department=$department&namedriver=$namedriver\" target=\"blank\" style=\"cursor:pointer;color:#43ad04;float:right;margin-top:10px;\" class=\"excel\"> <img src=\"image/excel.png\" title=\"Export to excel\" width=\"50px\" height=\"50\"></a> ");
			print("</div>");*/
	...
	$price = 0;
		...
		for($i=0;$i<$DateNum;$i++){
			...
			$getdata = getlist("SELECT * FROM production_order as p JOIN insertdata_transport  as i ON p.boonpon_id = i.boonpon_id WHERE delivery_date = '".$cut_new_date[0]."' AND i.nameDriver = '".$getdriver_name[0]['id_driver']."' and p.boonpon_id !='' GROUP BY p.boonpon_id");

			//print("SELECT * FROM production_order as p JOIN insertdata_transport  as i ON p.boonpon_id = i.boonpon_id WHERE delivery_date = '".$cut_new_date[0]."' AND i.nameDriver = '".$getdriver_name[0]['id_driver']."' and p.boonpon_id !='' GROUP BY p.boonpon_id");
		...
		print("<tr>");
			...
			print("<td align = \"center\" style = \"width:20mm;\" class='body-style'>");
						print printNumDate($cut_new_date[0]);//วันที่ส่ง
			print("</td>");	
			...
			if(empty($getdata)){
					...
					$get_note = getlist("SELECT * FROM driver_note WHERE driver_id = '".$getdriver_name[0]['id_driver']."' and note_date = '".$cut_new_date[0]."'");
								...
								print("<a href=\"report_boonpon2.php?datein=".$datein."&dateout=".$dateout."&department=$department&namedriver=$namedriver&delete_note=".$get_note[0]['note_id']."\" onclick=\"return confirm('คุณต้องการลบข้อมูลกาวนี้ใช่หรือไม่?')\">");
									if(empty($_GET['excel'])){
										print("<img src=\"image/delete.png\" title=\"ลบ\" width=\"12px\" height=\"12px\" class='not-show'>");
									}
