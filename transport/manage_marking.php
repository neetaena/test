...
<?php
query("USE transport");
if(!empty($_GET['delete_mark'])){
  query("DELETE FROM production_marking where marking_id='".$_GET['delete_mark']."'");
...
print("<div style=\"float:right;margin-bottom:10px;\">");
    ...
    print ("<label for=\"\">ค้นหาลาย</label>");
    print ("<input type = \"text\" name = \"search\" style = \"width:60mm;\">");
    print ("<input type = \"submit\" name = \"summit_data_mark\" value = \"ค้นหา\" >");
    print ("<input type = \"submit\" name = \"reset_data\" value = \"Clear\" >");
    print("</div>");
    print("<div style=\"float:left;padding:10px 20px 10px 10px;\">");
     print("<a onclick = \"window.open('add/add_marking.php' , '','nenuber=no,toorlbar=no,location=no,scrollbars=no, status=no,resizable=no,width=700,height=300,top=45 ,left=250') \" style=\"cursor:pointer;color:blue;font-size:25px;\"><i class=\"ace-icon fa fa-plus-square\"></i> Add</a> ");
    print("</div>");
print ("</form>");
      ...
      $offset = ($currentpage - 1) * $rowsperpage;
             ...
             $sql =getlist("SELECT * from production_marking ");
      ...
      if(empty($_POST['summit_data_mark']))
      {
     		$query_data =getlist("SELECT * from production_marking as p inner join type_production as t on p.product_id=t.id_production  limit $currentpage1,$rowsperpage ");
			
			
      }
      else
      {  
              $query_data =getlist("SELECT * from  production_marking as p inner join type_production as t on p.product_id=t.id_production where mark_name like '%".$_POST['search']."%'");
        ...
        print("<table style=\"width:100%;empty-cells: show;font-size:18px;margin-bottom:50px;\" border = \"1\" cellspacing = \"0\" cellpadding = \"0\" align = \"center\" valign = \"middle\" border: 1px groove;\">");
                ...
                for($i=0;$i<sizeof($query_data);$i++)
                {
                  ...
                  print("<tr class=\"body_table\" style=\"border:1px groove;\">");
                    print("<td style = \"width:20%;padding-left: 5px;\" >");
                      print($query_data[$i]['mark_name']);//เลขที่ใบสั่งขาย
                    print("</td>");
                    ...
                    print("<a onclick = \"window.open('edit/edit_marking.php?marking_id=".$query_data[$i]['marking_id']."' , '','nenuber=no,toorlbar=no,location=no,scrollbars=no, status=no,resizable=no,width=700,height=300,top=45 ,left=250') \" style=\"cursor:pointer;color:#43ad04;\"><img src=\"image/edit_2.png\" title=\"แก้ไข\" width=\"30px\" height=\"30px\"></a> ");
                    print("</td>");

                     print("<td style = \"width:5%;text-align:center;\">");
                     print("<a href=\"index.php?path=manage_plate&currentpage=".$_GET['currentpage']."&delete_mark=".$query_data[$i]['marking_id']."\" onclick=\"return confirm('คุณต้องการลบข้อมูลกาวนี้ใช่หรือไม่?')\"><img src=\"image/delete.png\" title=\"ลบ\" width=\"30px\" height=\"30px\"></a>");
                    print("</td>");   
            ...
            if(empty($_POST['reset_data']) && empty($_POST['search']))
            {
