...
<html>
	...
	?>
		<table  bgcolor = "#FFFFFF" border = "1" cellspacing = "0" cellpadding = "2" align = "center" valign = "middle" style="width:300mm;empty-cells: show;border: 1px groove;">
				<tr>
					...
					<?php
						$not_show = array(4,8,9,11,12,17,18,19,20,21,22);
	...
	$get_type = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production WHERE $where_data  group by product_id ");
			...
			$not_show = array(4,6,7,8,9,11,12,17,18,19,20,21,22,15);
			...
			{
				$m = 1;
				$get_new_invoice = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production  WHERE warehouse_id ='".$getdata1[$i]['warehouse_id']."' and  product_id='".$get_type[$ty]['product_id']."' and  delivery_date='$id' order by invoice asc");
				...
				$total_court=0;
				$total_q = 0;
				if(!empty($get_new_invoice) && $getdata1[$i]['warehouse_id'] !=""){

					for ($t=0; $t < sizeof($get_new_invoice); $t++) { 
						$get_new = getlist("SELECT * FROM production_order as po inner join type_production as tp on product_id=id_production  WHERE warehouse_id ='".$getdata1[$i]['warehouse_id']."' and  product_id='".$get_type[$ty]['product_id']."' and  delivery_date='$id' and invoice='".$get_new_invoice[$t]['invoice']."'");
