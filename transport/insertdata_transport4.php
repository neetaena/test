	...
	if(!empty($_POST['summit'])){
		...
		if(!empty($_POST['car']) && $_POST['namedriver']){
			...
			$a = query("INSERT into insertdata_transport SET number = '".$_GET['id']."'
						...
						,status = '1',boonpon_id='".$_POST['boonpon_id']."',up='$up',down='$down',final='$final',type_customer='$type_customer',up_down_rate='$vngb_id'");
...
<table bgcolor = "#aaaaaa" border = "1" cellspacing = "0" cellpadding = "2" align = "center" valign = "middle" style="width:180mm;height:10mm;empty-cells: show;background-color: #aaa;margin-top: 6px;">
	<form action = "" name = "insertquarity" method = "POST">
		...
		<tr>
			...
			<td align = "center" class="body_table">
				<?php
					$get_customer = getlist("SELECT * FROM production_order as po inner join customer as c on po.name=c.id_customer WHERE warehouse_id='".$_GET['warehouse_id']."' AND delivery_date='".$_GET['datedata']."'");
					//print("SELECT * FROM production_order as po inner join customer as c on po.name=c.id_customer WHERE number='".$_GET['id']."'");
				?>
...
//---------------------------------------------------- กำหนดค่า ---------------------------------------------------
		...
		<tr>
			<td align = "center" class="body_table">
				<b>รถบ้านบึงมาจากลูกค้า <t style='color: red'>(ถ้ามี)</t></b>
			</td>
			<td align = "center" class="input_data2">
				
				<?php
					
						print("<input type = \"text\" name = \"sub_vngb_name\" id=\"sub_vngb_name\" value = \"".$_POST['sub_vngb_name']."\" style = \"width:90mm;height:10mm;empty-cells: show;font-family:angsana new;font-size:20px;\" onkeydown=\"if (event.keyCode == 13) { this.form.submit(); return false; }\" class='serach_vngb' onchange = \"this.form.submit();\">");
...
<?php
		...
		print("<tr>");
			...
			$datail = $get_fule[0]['typecar']==2 ? $query_data[$i]['averang'] : $query_data[$i]['average_taylor'];
			$up_down = $datail;
			$plus = strpos($up_down,"+");
			$minus = strpos($up_down,"-");
				...
				print("<input type = \"text\" name = \"up\" value = \"".$up_down."\" class=\"body_table\" style = \"text-align: center;font-weight: bold;\" onkeydown=\"if (event.keyCode == 13) { this.form.submit(); return false; }\" onchange = \"this.form.submit();\" OnKeyPress=\"return chkNumber(this)\" readonly>");
			print("</td>");
		print("</tr>");
		/*print("<tr>");
			print("<td align = \"center\" class=\"body_table\">");
			...
			print("</td>");
			print("<td align = \"center\" class=\"body_table\">");
				print("<input type = \"text\" name = \"down\" value = \"".$_POST['down']."\" class=\"body_table\" style = \"text-align: center;font-weight: bold;\"  onkeydown=\"if (event.keyCode == 13) { this.form.submit(); return false; }\" onchange = \"this.form.submit();\" OnKeyPress=\"return chkNumber(this)\" readonly>");
			print("</td>");
		print("</tr>");*/
		print("<tr>");
			print("<td align = \"center\" class=\"body_table\">");
			...
			//$_POST['final'] = ($_POST['standard']+$_POST['up'])-$_POST['down'];
				print("<input type = \"text\" name = \"final\" value = \"".$_POST['final']."\" class=\"body_table\" style = \"text-align: center;font-weight: bold;\"  onkeydown=\"if (event.keyCode == 13) { this.form.submit(); return false; }\" onchange = \"this.form.submit();\"  OnKeyPress=\"return chkNumber(this)\" readonly>");
			print("</td>");
		print("</tr>");
