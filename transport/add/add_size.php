	...
	<?php
	    ...
	    if(!empty($_POST['summit']))
	    {
	        ...
	        $size_orderby = $_POST['size_orderby'];
	            ...
	            $check_data = getlist("SELECT * FROM production_size WHERE size_description='$size_data' and product_id='$product_id'");
	            if(empty($check_data))
	            {
	                $sql = query("INSERT INTO production_size SET size_description='$size_data',product_id='$product_id',size_orderby='$size_orderby'");
